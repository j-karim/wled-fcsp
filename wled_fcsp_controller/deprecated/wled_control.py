...
from wled_fcsp_controller.apis.deprecated.football_api import FootballApi
...
def main(ip_address: str, football_api_key: str):
    ...
    print(f'API key: {football_api_key}')
    ...
    while True:
        if upcoming_game_id is not None or debug:
            ...
            if status == 'Not Started':
                ...
                print(f'Match is running, current score: {updated_score}')
            ...
            print('Next match details:')
            print(json.dumps(next_match['response'][0]['teams'], indent=4))
            print(json.dumps(next_match['response'][0]['fixture'], indent=4))
            print(f'Sleeping until start of next match ({match_datetime}!')
            time.sleep((match_datetime - now).seconds)
